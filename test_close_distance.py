...
import math  # <-- added for haversine & trig
...
# New globals to represent the final optimal point (set after optimization completes)
_OPTIMAL_LAST_POINT: Optional[Tuple[float, float]] = None  # (lat, lon)
_OPTIMAL_MAX_DIST: float = None  # meters; used for scaling simulate_rsrp / simulate_signal_quality
...
SIM_MIN_UES_FAR        = 3       # UE count when far
...
def sim_distance_m(lat1, lon1, lat2, lon2) -> float:
    # rough meters using lat/lon deltas (existing helper)
    dy = degrees_to_meters(lat2 - lat1)
    dx = degrees_to_meters(lon2 - lon1, is_longitude=True, center_lat=(lat1+lat2)/2)
    return float(np.hypot(dx, dy))

# ---------- New: haversine & distance-based simulator functions (user-provided formulas) ----------
...
def simulate_rsrp(distance, max_dist):
    ...
    # clamp distance and max_dist
    if max_dist is None or max_dist <= 0:
        max_dist = SIM_EFFECT_RADIUS_M
    d = float(min(max(distance, 0.0), max_dist))
    return float(-90.0 + 20.0 * (1.0 - (d / max_dist)))
...
def simulate_signal_quality(distance, max_dist):
    ...
    if max_dist is None or max_dist <= 0:
        max_dist = SIM_EFFECT_RADIUS_M
    d = float(min(max(distance, 0.0), max_dist))
    return float(50.0 + 50.0 * (1.0 - (d / max_dist)))

# ---------- Modified simulate_metrics ----------
def simulate_metrics(cur_lat, cur_lon, tgt_lat, tgt_lon) -> Tuple[float,int]:
    """
    Simulator now prefers to compute metrics with respect to the OPTIMAL_LAST_POINT
    (set when optimization completes). If OPTIMAL_LAST_POINT is not set, we fall
    back to distance to the provided target (legacy behavior).
    """
    global _OPTIMAL_LAST_POINT, _OPTIMAL_MAX_DIST

    if _OPTIMAL_LAST_POINT is not None:
        # use haversine distance to the optimal last point
        center_lat, center_lon = _OPTIMAL_LAST_POINT
        d = haversine(cur_lat, cur_lon, center_lat, center_lon)
        maxd = _OPTIMAL_MAX_DIST if _OPTIMAL_MAX_DIST is not None else SIM_EFFECT_RADIUS_M
        rsrp = simulate_rsrp(d, maxd)
        ues  = int(round(SIM_MIN_UES_FAR + np.clip(1.0 - (d / maxd), 0.0, 1.0) * (SIM_MAX_UES_AT_TARGET - SIM_MIN_UES_FAR)))
        return float(rsrp), int(ues)

    # legacy behavior: distance to target waypoint
...
class PolygonDroneOptimizer:
    ...
    def polygon_bisection_search_real(self, min_area_threshold=100):
        ...
        # final center (compute centroid of final polygon)
        center_lng = sum(lng for lng, lat in current) / 4
        center_lat = sum(lat for lng, lat in current) / 4

        # set global optimal point & max dist (used by the new simulate_metrics)
        global _OPTIMAL_LAST_POINT, _OPTIMAL_MAX_DIST
        _OPTIMAL_LAST_POINT = (center_lat, center_lng)
        # Set max distance to the farthest of the original initial corners -> used for scaling
        _OPTIMAL_MAX_DIST = max(haversine(center_lat, center_lng, lat, lng) for lng, lat in self.initial_corners)

        # perform spiral into the centroid and stay there
        self.spiral_into_center(center_lat, center_lng)

        # after spiral_into_center returns (it won't unless interrupted), still keep a final measure
    ...
    # ----- new spiral helper -----
    def spiral_into_center(self, center_lat, center_lon, max_radius_m=None, steps_per_rev=12, revolutions=4):
        """
        Generate a spiral of waypoints inward to the center and move the drone along them.
        After reaching the center, hover there indefinitely (publishing metrics periodically).
        - max_radius_m: outer radius to start spiral (if None, uses _OPTIMAL_MAX_DIST or SIM_EFFECT_RADIUS_M)
        - steps_per_rev: number of waypoints per revolution
        - revolutions: how many revolutions to make while spiraling inward
        """
        global _OPTIMAL_MAX_DIST
        if max_radius_m is None:
            max_radius_m = _OPTIMAL_MAX_DIST if _OPTIMAL_MAX_DIST is not None else SIM_EFFECT_RADIUS_M

        # Build spiral radii and angles
        total_steps = steps_per_rev * revolutions
        # radii decreasing from max_radius_m -> 0
        radii = np.linspace(max_radius_m, 0.0, total_steps + 1)
        waypoints = []
        for i, r in enumerate(radii):
            # angle increases to create spirals
            theta = (i / steps_per_rev) * (2.0 * math.pi)
            # convert meters -> lat/lon deltas
            dlat = r / 111111.0 * math.cos(theta)
            dlon = r / (111111.0 * math.cos(math.radians(center_lat))) * math.sin(theta)
            wp_lat = center_lat + dlat
            wp_lon = center_lon + dlon
            waypoints.append((wp_lat, wp_lon))

        print(f"Spiral: {len(waypoints)} waypoints from {max_radius_m:.1f} m -> center")

        # Move along spiral waypoints (use move_step_by_step to leverage graph routing + measurements)
        for idx, (lat, lon) in enumerate(waypoints):
            label = f"spiral_{idx}"
            print(f"Spiral step {idx+1}/{len(waypoints)} -> ({lat:.6f},{lon:.6f})")
            try:
                # Using move_step_by_step so pathfinding & measurements are consistent
                self.move_step_by_step(lat, lon, label)
                # quick measurement at new position
                self.measure_here(lat, lon, label)
            except Exception as e:
                print(f"[WARN] Spiral move/measure failed at step {idx}: {e}")
            time.sleep(0.15)

        # final move to exact centroid
        print("Finalizing: moving to exact centroid and hovering.")
        self.move_step_by_step(center_lat, center_lon, "spiral_final_center")
        self.measure_here(center_lat, center_lon, "spiral_final_center")

        # Hover indefinitely at center, publishing periodic updates so dashboard / ROS2 have live data.
        print("Hovering indefinitely at optimal point (CTRL-C to stop)...")
        while True:
            # publish and update dash with current known last metrics or simulator
            # reuse measure_here which will call simulator if necessary
            self.measure_here(center_lat, center_lon, "hover_optimal_center")
            time.sleep(2.0)
# ==================================================
